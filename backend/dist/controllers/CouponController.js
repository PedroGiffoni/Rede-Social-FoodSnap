...
class CouponController {
    ...
    */
    async create(req, res) {
        try {
            const { businessProfileId, title, description, code, discountType, discountValue, validUntil, } = req.body;
            ...
            return res.status(400).json({
                message: error instanceof Error ? error.message : "Erro ao criar cupom.",
            });
        }
    }
    /*
    ...
    */
    async listByBusiness(req, res) {
        try {
            ...
            const couponService = new CouponService_1.CouponService();
            const coupons = await couponService.listCouponsByBusiness(businessProfileId);
            return res.status(200).json(coupons);
        }
        catch (error) {
            return res.status(400).json({
    ...
    */
    async toggle(req, res) {
        try {
            ...
            const couponService = new CouponService_1.CouponService();
            const coupon = await couponService.toggleCouponStatus(req.user?.id, couponId);
  ...
  */
    async update(req, res) {
        try {
            ...
            return res.status(400).json({
                message: error instanceof Error ? error.message : "Erro ao editar cupom.",
            });
        }
    }
    /*
  ...
  */
    async delete(req, res) {
        try {
            const { couponId } = req.params;
            const couponService = new CouponService_1.CouponService();
            const result = await couponService.deleteCoupon(req.user?.id, couponId);
            return res.status(200).json(result);
        }
        catch (error) {
            return res.status(400).json({
                message: error instanceof Error ? error.message : "Erro ao excluir cupom.",
            });
        }
    }
}
