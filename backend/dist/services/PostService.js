...
class PostService {
    ...
    */
    validateRating(value, fieldName) {
        ...
        if (hasMoreThanOneDecimal) {
            throw new Error(`A nota ${fieldName} deve ter no máximo uma casa decimal.`);
        }
    }
    /*
      Calcula a média das seis notas principais.
    */
    calculateAverageRating(recommendationRating, priceRating, flavorRating, presentationRating, serviceRating, environmentRating) {
        const average = (recommendationRating +
            priceRating +
            flavorRating +
            presentationRating +
            serviceRating +
    ...
    */
    ...
    async createPost(data) {
        ...
        if ((author.userType === "COMMON" || author.userType === "INFLUENCER") &&
            data.postType !== "REVIEW") {
            throw new Error("Usuários comuns e influenciadores só podem criar avaliações.");
        }
        ...
        if (data.postType === "REVIEW" && !businessProfileId) {
            ...
            const existingBusiness = await this.businessRepository.findExactByName(data.businessName);
            if (existingBusiness) {
        ...
        if (data.postType === "REVIEW") {
            ...
            averageRating = this.calculateAverageRating(data.recommendationRating, data.priceRating, data.flavorRating, data.presentationRating, data.serviceRating, data.environmentRating);
        }
    ...
    */
    ...
    async listPostsByBusiness(businessProfileId) {
        if (!businessProfileId) {
  ...
  */
    async listFollowingFeed(userId) {
        if (!userId) {
